...
function loadStatsPage(){
	...
	for(row_counter = 2; row_counter < table.rows.length; row_counter++){//Outer for loop iterates over each row
		away_score = table.rows[row_counter].cells["2"].innerHTML;//Read in a cells current value
		home_score = table.rows[row_counter].cells["3"].innerHTML;
...
function switchPlayers(playerNum){
	...
	document.getElementById('player_img').setAttribute('src', players[playerNum].alt);
	...
	var avgpass = avg_pass.toFixed(2);
	var avgrush = avg_rush.toFixed(2);
	var avgrec = avg_rec.toFixed(2);
